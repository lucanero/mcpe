    ...
    else
    {
        ...
        if (var8 > 0.15f) {
            ...
            for (int var14 = 0; var14 < 1.0f + var8 * 60.0f; ++var14) {
                float var15 = (sharedRandom.nextFloat() * 2.0f - 1.0f);
                float var17 = (sharedRandom.nextInt(2) * 2 - 1) * 0.7f;
                float var19;
                float var21;
                if (sharedRandom.nextInt(1))
                {
        ...
        Entity::Vector var18 = m_pLevel->getEntities(this, hit);
        if (var18.size() > 0)
        {
            for (Entity::Vector::iterator it = var18.begin();  it != var18.end(); ++it)
            {
                Entity* entity = *it;
